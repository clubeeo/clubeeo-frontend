...
export const eventSourceOptions: Array<IOption> = [
  {
    label: 'badges',
    value: 'badge',
    caption: 'granting badges, achievements or scorepoints',
    children: [
      {
        label: 'badge granted',
        value: 'badge:granted',
        caption: 'triggers on first assigment of a type',
      },
      {
        label: 'badge changed',
        value: 'badge:changed',
        caption:
          'triggers on any change, including adding scorepoints and secondary assignments',
      },
    ],
  },
  {
    label: 'roles',
    value: 'role',
    caption: 'granting or removal roles',
    children: [
      {
        label: 'role granted',
        value: 'role:granted',
        caption: 'triggers on assigment of a role',
      },
      {
        label: 'role removed',
        value: 'role:removed',
        caption: 'triggers on role removal',
      },
    ],
  },
  {
    label: 'tokens',
    value: 'token',
    caption: 'NFT or fungible tokens events',
    children: [
      {
        label: 'NFT received',
        value: 'nft:received',
...
export const eventSourceOptionsMap = Object.fromEntries<IOption>(
  eventSourceOptions.map((eso: IOption) => [eso.value, eso])
);
...
for (const sourceOption of eventSourceOptions) {
  for (const typeOption of sourceOption.children || []) {
    eventTypeOptionsMap[typeOption.value] = {
      ...typeOption,
      parent: { value: sourceOption.value },
    };
  }
}
...
export const processingTypeOptions: Array<IProcessingTypeOption> = [
  {
    label: 'bypass',
    value: 'bypass',
    caption: 'send event outputs to action inputs without modification',
  },
  ...
  // { label: 'merge', value: 'merge', caption: 'merges data from event and trigger', },
  {
    label: 'javascript',
    value: 'javascript',
    caption: 'low-code / lambda',
    showCode: true,
  },
];
export const processingTypeOptionsMap =
  Object.fromEntries<IProcessingTypeOption>(
    processingTypeOptions.map((eso: IProcessingTypeOption) => [eso.value, eso])
  );
    ...
    children: [
      {
        label: 'badge grant',
        value: 'badge:grant',
        caption: 'assigment a badge of a type',
      },
    ],
  },
  {
...
export const taskTargetOptionsMap = Object.fromEntries<IOption>(
  taskTargetOptions.map((tto: IOption) => [tto.value, tto])
);
...
for (const targetOption of taskTargetOptions) {
  for (const typeOption of targetOption.children || []) {
    taskTypeOptionsMap[typeOption.value] = {
      ...typeOption,
      parent: { value: targetOption.value },
    };
  }
}
